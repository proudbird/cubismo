...
Tools.Dialog.alert = function(config) {
  Window().Client.emit('dialog', {type: 'alert', config: config});
}

Tools.Dialog.message = function(config) {
  Window().Client.emit('dialog', {type: 'message', config: config});
}

Tools.Dialog.waitingCursor = function(state) {
  Window().Client.emit('dialog', {type: 'waitingCursor', config: state});
}
...
Tools.Dialog.notify = function(title, options, callback) {
  ...
  Window().Client.emit('dialog', {type: 'notify', title: title, options: options}, function(error) {
    Callback(callback);
  });
}
